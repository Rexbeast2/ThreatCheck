...
def cli():
    argparser = ArgumentParser(
        description=" ThreatCheck | A project for scanning "
    ...
    reportargs.add_argument(
        "-o",
        "--output",
        help="Output file name. (Default : ThreatCheck.log)",
        default="ThreatCheck",
        type=str,
        required=False,
    )
...
def InitArgsConf(args, log) -> None:
    ...
    try:
        ...
        if config.has_option("ThreatCheck", "target"):
            args.target = config.get("ThreatCheck", "target").lower()

        if config.has_option("ThreatCheck", "hostfile"):
            args.host_file = config.get("ThreatCheck", "hostfile").lower()

        if config.has_option("ThreatCheck", "scantype"):
            args.scan_type = config.get("ThreatCheck", "scantype").lower()

        if config.has_option("ThreatCheck", "nmapflags"):
            args.nmap_flags = config.get("ThreatCheck", "nmapflags").lower()

        if config.has_option("ThreatCheck", "speed"):
            try:
                args.speed = int(config.get("ThreatCheck", "speed"))
            except ValueError:
                log.logger("error", "Invalid speed value in config file. (Default : 3)")

        if config.has_option("ThreatCheck", "apikey"):
            args.api = config.get("ThreatCheck", "apikey").lower()

        if config.has_option("ThreatCheck", "auto"):
            args.yes_please = True

        if config.has_option("ThreatCheck", "mode"):
            args.mode = config.get("ThreatCheck", "mode").lower()

        if config.has_option("ThreatCheck", "noisetimeout"):
            args.noise_timeout = config.get("ThreatCheck", "noisetimeout").lower()
